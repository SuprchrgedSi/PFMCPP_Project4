...
struct DoubleType
{
    ...
    // explicit DoubleType(double v) : value(std::make_unique<double>(v))  {}
    ~DoubleType()
    {
        delete value;
    }
...
struct FloatType
{
    ...
    FloatType& apply(std::function<FloatType&>());
    FloatType& apply(void(*)());
...
struct IntType
{
    ...
    IntType& apply(std::function<IntType&>());
    IntType& apply(void(*)());
